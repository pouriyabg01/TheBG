    ...
    <div class="pageCartItem" data-product-id="{{ $product->id }}">
        <div class="d-sm-flex justify-content-between align-items-center my-2 pb-3 border-bottom">
            ...
            <div class="pt-2 pt-sm-0 ps-sm-3 mx-auto mx-sm-0 text-center text-sm-start" style="max-width: 9rem;">
                ...
                <input class="form-control" type="number" id="quantity" min="1" value="{{ $product->pivot->quantity }}">
                <button class="btn btn-link px-0 text-danger" type="button" onclick="dellItemFromCart(this)" data-route="{{ route('remove-cart' , $product) }}" data-csrf="{{ csrf_token() }}"><i class="ci-close-circle me-2"></i><span class="fs-sm">Remove</span></button>
            </div>
        </div>
    </div>
@endforeach
        ...
        <div class="bg-white rounded-3 shadow-lg p-4">
            <div class="py-2 px-xl-2">
                <div class="text-center mb-4 pb-3 border-bottom">
                    ...
                    <h3 class="fw-normal" id="sideBarCartItemSubTotal">{{ $customer->cartItemSubTotal() }}</h3>
                </div>
                <div class="mb-3 mb-4">
                ...
                </div><a class="btn btn-primary btn-shadow d-block w-100 mt-4" href="{{ route('checkout-detail') }}"><i class="ci-card fs-lg me-2"></i>Proceed to Checkout</a>
            </div>
        </div>
    </aside>
@endsection
